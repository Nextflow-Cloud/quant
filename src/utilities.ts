...
export const dynamicImport = async (path: string) => (await import(path)).default;

export const resolveCommands = async () => {
    const files = await readdir(join(__dirname, "/commands"));
    const events = await Promise.all(files.map(file => dynamicImport(pathToFileURL(join(__dirname, `/commands/${file}`)).href) as Promise<Command>));
    return events;
};

export const resolveEvents = async () => {
    const files = await readdir(join(__dirname, "/events"));
    const events = await Promise.all(files.map(file => dynamicImport(pathToFileURL(join(__dirname, `/events/${file}`)).href) as Promise<Event<keyof ClientEvents>>));
    return events;
};
